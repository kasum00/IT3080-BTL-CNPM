  ...
  try {
    ...
    const [daThuResult] = await sequelize.query(
      `SELECT IFNULL(SUM(ThanhTien), 0) as daThu 
       FROM KhoanThuTheoHo 
       WHERE TrangThai IN ('Đã đóng', 'da_thu', 'ĐÃ ĐÓNG')`
    );
    ...
    const [hoDaDongDuResult] = await sequelize.query(
      `SELECT COUNT(DISTINCT MaHoKhau) as soHo
       ...
       WHERE NOT EXISTS (
         ...
         AND kthk.TrangThai NOT IN ('Đã đóng', 'da_thu', 'ĐÃ ĐÓNG')
       )
       AND EXISTS (
         SELECT 1 
    ...
    const [hoChuaDongDuResult] = await sequelize.query(
      `SELECT COUNT(DISTINCT MaHoKhau) as soHo
       FROM KhoanThuTheoHo
       WHERE TrangThai NOT IN ('Đã đóng', 'da_thu', 'ĐÃ ĐÓNG')`
    );
    ...
    const [theoLoaiResult] = await sequelize.query(
      `SELECT 
         ...
         IFNULL(SUM(CASE WHEN kthk.TrangThai IN ('Đã đóng', 'da_thu', 'ĐÃ ĐÓNG') THEN kthk.ThanhTien ELSE 0 END), 0) as daThu
       FROM KhoanThu kt
  ...
  try {
    const [result] = await sequelize.query(
      `SELECT 
         ...
         IFNULL(SUM(kthk.ThanhTien), 0) as tongTien,
         IFNULL(SUM(CASE WHEN kthk.TrangThai IN ('Đã đóng', 'da_thu', 'ĐÃ ĐÓNG') THEN kthk.ThanhTien ELSE 0 END), 0) as daThu,
         SUM(CASE WHEN kthk.TrangThai IN ('Đã đóng', 'da_thu', 'ĐÃ ĐÓNG') THEN 1 ELSE 0 END) as soHoDaDong,
         SUM(CASE WHEN kthk.TrangThai NOT IN ('Đã đóng', 'da_thu', 'ĐÃ ĐÓNG') OR kthk.TrangThai IS NULL THEN 1 ELSE 0 END) as soHoChuaDong
       FROM KhoanThu kt
       LEFT JOIN KhoanThuTheoHo kthk ON kt.MaKhoanThu = kthk.MaKhoanThu
  ...
  try {
    const [result] = await sequelize.query(
      `SELECT 
         ...
         SUM(CASE WHEN kthk.TrangThai IN ('Đã đóng', 'da_thu', 'ĐÃ ĐÓNG') THEN 1 ELSE 0 END) as soDaDong,
         SUM(CASE WHEN kthk.TrangThai NOT IN ('Đã đóng', 'da_thu', 'ĐÃ ĐÓNG') OR kthk.TrangThai IS NULL THEN 1 ELSE 0 END) as soChuaDong
       FROM HoKhau hk
