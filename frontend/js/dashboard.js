...
=======
  ...
  const btnOpenEdit = document.getElementById("btn-open-edit");
  const closeEditBtn = document.getElementById("closeModal");
  const memberEl = document.getElementById("count-people");

  fetch("http://localhost:3000/api/nhan-khau/count")
    .then((res) => res.json())
    .then((data) => {
      if (memberEl) {
        memberEl.innerText = `${data.total} người`;
      }
    })
    .catch((err) => {
      console.error("Lỗi lấy số dân:", err);
      memberEl.innerText = "Lỗi dữ liệu";
    });
  /* ===============================
       MỞ MODAL CHI TIẾT
       =============================== */
